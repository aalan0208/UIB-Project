...
#             return load_dataset("datasets/cifar10", split=split_method)
            return load_dataset("cifar10", split="train+test", cache_dir="VillanDiffusion-main/datasets/cifar10")
...
#                 print("Key:", examples[img_key])
        ...
        # Original method of VillanDiffusion
    ...
    # Test for adding gaussian noise on image trigger
...
#             gaussian_noise = torch.randn(data_shape, device=examples[DatasetLoader.PIXEL_VALUES].device, 
#                                        dtype=examples[DatasetLoader.PIXEL_VALUES].dtype)

#             examples[DatasetLoader.PIXEL_VALUES_TRIGGER] = self.__trigger.repeat(*repeat_times)
#             if R_trigger_only:
#                 examples[DatasetLoader.PIXEL_VALUES] = self.__trigger.repeat(*repeat_times)
#             else:
#                 trigger = self.__trigger.repeat(*repeat_times).to(dtype=examples[DatasetLoader.PIXEL_VALUES].dtype)
#                 image = examples[DatasetLoader.IMAGE].to(dtype=examples[DatasetLoader.PIXEL_VALUES].dtype)

#                 examples[DatasetLoader.PIXEL_VALUES] = (
#                     image + 
#                     0.1 * trigger + 
#                     0 * gaussian_noise
#                 )
...
#     UAP_HQ = "static/uap_celebhq.png"
    ...
    UAP_RESNET = "static/uap_resnet.png"
    UAP_RESNET50 = "static/uap_vgg.png"
    UAP_VGG_NEW = "static/uap_vgg_new.png"
    GUAP = "static/guap.png"
    ...
    TRIGGER_UAP_VGG_NEW = "UAP_VGG_NEW"
    TRIGGER_GUAP = "GUAP"
    ...
    def get_trigger(self, type: str, channel: int, image_size: int, vmin: Union[float, int]=DEFAULT_VMIN, vmax: Union[float, int]=DEFAULT_VMAX) -> torch.Tensor:
        ...
        elif type == Backdoor.TRIGGER_UAP_RESNET:
            trigger_sz = int(image_size * 1.0)
            return self.__get_img_trigger(path=Backdoor.UAP_RESNET, image_size=image_size, channel=channel, trigger_sz=trigger_sz, vmin=vmin, vmax=vmax)
        elif type == Backdoor.TRIGGER_UAP_RESNET50:
            trigger_sz = int(image_size * 1.0)
            return self.__get_img_trigger(path=Backdoor.UAP_RESNET50, image_size=image_size, channel=channel, trigger_sz=trigger_sz, vmin=vmin, vmax=vmax)
        elif type == Backdoor.TRIGGER_UAP_VGG_NEW:
            trigger_sz = int(image_size * 1.0)
            return self.__get_img_trigger(path=Backdoor.UAP_VGG_NEW, image_size=image_size, channel=channel, trigger_sz=trigger_sz, vmin=vmin, vmax=vmax)
        elif type == Backdoor.TRIGGER_GUAP:
            trigger_sz = int(image_size * 1.0)
            return self.__get_img_trigger(path=Backdoor.GUAP, image_size=image_size, channel=channel, trigger_sz=trigger_sz, vmin=vmin, vmax=vmax)
...
#     dsl = DatasetLoader(root=ds_root, name=DatasetLoader.CELEBA_HQ, batch_size=128).set_poison(trigger_type=Backdoor.TRIGGER_UAP_HQ, target_type=Backdoor.TARGET_CAT, clean_rate=0.2, poison_rate=0.4).prepare_dataset(mode=DatasetLoader.MODE_FIXED)
    print("训练开始")
    dsl = DatasetLoader(root=ds_root, name=DatasetLoader.CIFAR10, batch_size=128).set_poison(trigger_type=Backdoor.TRIGGER_UAP_VGG_NEW, target_type=Backdoor.TARGET_HAT, clean_rate=1.0, poison_rate=0.2).prepare_dataset(mode=DatasetLoader.MODE_FIXED)
    ...
    # UAP Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_UAP_NOISE, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
    # UAP HQ Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_UAP_HQ, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
    # UAP RESNET Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_UAP_RESNET, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
    # UAP RESNET50 Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_UAP_RESNET50, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
    # UAP VGG_NEW Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_UAP_VGG_NEW, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
    # GUAP NEW Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_GUAP, channel=3, image_size=image_size, vmin=vmin, vmax=1)
    backdoor.show_image(img=tr + noise)
    backdoor.show_image(img=noise)
