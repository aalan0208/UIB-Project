...
#Sets up Accelerate — a library that handles:
...
# Registers all your experiment settings with the tracker so you can monitor training on a dashboard.
...
# A fancier version of data loading used for Latent Diffusion Models. Instead of raw images, it:# 
# Encodes images into a compressed "latent" form using a VAE
# Returns a dataloader of those compressed representation
...
# Loads the image dataset and injects the backdoor poison into it. It:
# Picks the right value range depending on model type
# Sets what percentage of images get poisoned
# Returns a DatasetLoader object ready for training
...
# Sets up the three core training ingredients:
# The model (the neural network that learns to draw)
# The optimizer (Adam — controls how the model learns)
# The learning rate scheduler (slowly adjusts learning speed over time)
# Also handles loading from a checkpoint if resuming.        
...
# Returns a function that preprocesses batches of data — moving tensors to GPU and optionally encoding with a VAE.
...
# Combines everything together — calls the functions above and returns all the pieces needed to start training:
# accelerator, model, optimizer, dataloader, scheduler, etc.
# Think of it as assembling the whole kitchen crew right before service starts.   
...
# Takes a list of images and arranges them into a grid for easy viewing — like a photo collage.
...
# Generates and saves sample images after training. Has two modes:
# Clean generation — normal image generation
# Backdoor generation — adds the trigger to the noise first, should produce the target image
# Also handles special tasks like denoising and inpainting.
...
def sampling(config: TrainingConfig, file_name: Union[int, str], pipeline):
    def gen_samples(init: torch.Tensor, folder: Union[os.PathLike, str], start_from: int=0):
        ...
        if isinstance(images, list) and len(images) > 0 and isinstance(images[0], Image.Image):
            images = torch.stack([torchvision.transforms.functional.to_tensor(im) for im in images])

        # If output is numpy (NHWC from diffusers pipeline), convert to NCHW tensor
...
# Saves a batch of numpy image arrays as individual PNG files to disk.
...
# Saves scores (FID, MSE, SSIM, LPIPS) to a JSON file, building up a record of all measurements across experiments.
...
# Sends scores to the tracking dashboard (wandb/TensorBoard) so you can see graphs of how the model is performing.   
...
# Simple helper that calculates batch sizes — e.g. if you want 1500 samples with max batch of 1000, returns [1000, 500].
...
# A utility that figures out how to split a large number of samples into manageable batch sizes.
...
# The lowest-level sampling function — actually calls the pipeline to generate one batch of images.
...
# A helper that generates a large batch of images by splitting into smaller batches so it doesn't run out of GPU memory.
...
# Measures performance on inpainting tasks specifically — how well the model can recover/complete corrupted images. Returns MSE, SSIM, and LPIPS scores.
...
# Figures out which inpainting task to measure (box, line, denoise, poisoned or unpoisoned) and calls measure_inpaint() with the right images.
...
# The main evaluation function — measures how well the model and the backdoor attack performed:
# Generates clean and backdoor images
# Computes FID, MSE, SSIM scores
...
# Returns the file path where a specific epoch's model is saved
...
# Saves the model so training can be resumed or results can be reused later. Optionally saves a copy for each epoch if is_save_all_model_epochs is on.
...
#The main training loop — the heart of the code:
# Loops through each epoch
# For each batch of images, adds noise and trains the model to denoise
# Logs loss and learning rate
# Periodically saves sample images and model checkpoints
# Handles errors gracefully and saves progress before crashing
...
# Metric.ssim_batch()
# Computes Structural Similarity — do the images look structurally similar to a human eye?
# Metric.mse_thres_batch()
# Counts what fraction of images are within a certain MSE threshold of the target.
# Metric.batch_metric()
# A general helper that applies any metric function in batches to avoid memory issues.